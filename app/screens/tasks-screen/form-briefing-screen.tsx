...
export interface FormBriefingScreenProps extends NavigationScreenProps<{}> {
}

export const FormBriefingScreen: React.FunctionComponent<FormBriefingScreenProps> = observer((props) => {
      ...
      */
      ...
      if( (!selectedAssignee || selectedAssignee.length == 0) || taskTitle == "" || taskDescription == "" || selectedStartDate == "" || selectedEndDate == "" || !selectedProject || selectedProject == ""){
        ...
        if(!selectedAssignee || selectedAssignee.length == 0){
          Toast.show("Anda harus memilih penerima briefing");
        }
        else if(taskTitle == ""){
          Toast.show("Anda harus mengisi judul briefing");
        }
        else if(taskDescription == ""){
          Toast.show("Anda harus mengisi deskripsi briefing");
        }
        else if(selectedStartDate == ""){
          Toast.show("Anda harus memilih tanggal mulai briefing");
        }
        else if(selectedEndDate == ""){
          Toast.show("Anda harus memilih tanggal selesai briefing");
        }
      ...
      // else if(taskRelated && !selectedRelated && selectedRelated.length > 0){
        ...
        Toast.show("Anda harus memilih briefing yang bersangkutan");
        validate = false;
      }
        ...
        Toast.show("Anda harus mengisi kolom diulang briefing dengan benar");
        validate = false;
      }
      // else if((spesifik)&&(array.length==0)){
      //   Toast.show("Anda harus memilih penerima briefing secara spesifik");
      //   validate = false;
      // }
      else{
        validate = true;
    ...
    const doAdd = async (param) => {
      ...
      if(result.kind != "ok"){
          ...
          rootStore.pushData('my_queues', {
              ...
              action: 'createTask',
              will_update: "tasks",
              description: "Menambahkan briefing",
              data: JSON.stringify(param)
          });
      }

      if(result.kind == "ok" && result.data){
        Toast.show("Anda berhasil menambahkan briefing");
        goBack();
    ...
    const doEdit = async (param) => {
      ...
      if(result.kind == "ok" && result.data){
        Toast.show("Anda berhasil mengubah informasi briefing");
        goBack();
        props.navigation.state.params.onBack();
      }
    }
    ...
    const SECTIONS = [
        {
            title: 'Isi detail briefing',
            slug: "detail-briefing"
        },
        {
            title: 'Opsi briefing',
            slug: "opsi-briefing"
    ...
    const _renderContent = section => {
        return (
          <View style={{ ...layout.accordion.content.wrapper }}>
            {(section.slug == 'detail-briefing' &&
              _renderDetail()
            )}
            {(section.slug == 'opsi-briefing' &&
              _renderOpsi()
            )}
          ...
          {(!spesifik) && 
            <View style={{ ...layout.textbox.wrapper, ...layout.textbox.outline, marginBottom:10 }}>
              ...
              <Picker
                  ...
                  style={{ ...layout.dropdown.input }}
              >
                  <Picker.Item label="Pilih Penerima briefing" value="" />
      ...
      return(
        <View>
          <TouchableOpacity style={layout.checkbox.wrapper} onPress={() => setTaskCC(!taskCC)}>
              ...
              <Text style={layout.checkbox.text}>briefing ini dilaporkan ke lainnya</Text>
          </TouchableOpacity>
          {(taskCC &&
            <View>
              <View style={{ ...layout.textbox.wrapper, ...layout.textbox.outline }}>
          ...
          {/* <TouchableOpacity style={layout.checkbox.wrapper} onPress={() => setTaskRelated(!taskRelated)}>
              ...
              <Text style={layout.checkbox.text}>briefing ini berkaitan dengan briefing lainnya</Text>
          </TouchableOpacity> */}
          {/* {(taskRelated && 
            <View style={{ ...layout.textbox.wrapper, ...layout.textbox.outline }}>
                <Icon name="ios-lock" style={{ ...layout.textbox.icon }} size={20} /> */}
                ...
                <Picker
                    ...
                    style={{ ...layout.dropdown.input }}
                >
                    <Picker.Item label="Pilih briefing" value="" />
                ...
                */}
                ...
                {/* <View style={layout.dropdown.input}>
                  <SectionedMultiSelect
                    ...
                    displayKey={"heading"}
                    subKey="children"
                    selectText="Pilih briefing"
                    searchPlaceholderText={"Cari briefing..."}
                    showDropDowns={false}
                    readOnlyHeadings={true}
                    onSelectedItemsChange={(selectedItems) => {
          ...
          <TouchableOpacity style={layout.checkbox.wrapper} onPress={() => setTaskInterval(!taskInterval)}>
              ...
              <Text style={layout.checkbox.text}>briefing ini perlu dilaporkan secara berkala</Text>
          </TouchableOpacity>
          {(taskInterval &&
            <View style={{ ...layout.textbox.wrapper, ...layout.textbox.outline }}>
          ...
          <TouchableOpacity style={layout.checkbox.wrapper} onPress={() => setTaskGPS(!taskGPS)}>
              ...
              <Text style={layout.checkbox.text}>briefing ini memerlukan akses GPS</Text>
          </TouchableOpacity>
          <TouchableOpacity style={layout.checkbox.wrapper} onPress={() => setTaskCamera(!taskCamera)}>
              ...
              <Text style={layout.checkbox.text}>briefing ini memerlukan akses ke kamera</Text>
          </TouchableOpacity>
            ...
            <View>
              ...
              {(!rootStore.settings.offline_mode &&
                <TouchableOpacity style={layout.checkbox.wrapper} onPress={() => setTaskRepeat(!taskRepeat)}>
                    ...
                    <Text style={layout.checkbox.text}>Ulangi briefing</Text>
                </TouchableOpacity>
              )}

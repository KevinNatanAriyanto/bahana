...
export const TasksScreen: React.FunctionComponent<TasksScreenProps> = observer((props) => {
          ...
          <ScrollView 
            ...
            <View style={{ ...layout.container.wrapper, ...layout.container.bodyView }}>
              ...
              {datas.map((item,i) => {
                ...
                return(
                  <TouchableOpacity key={slugnow+item.id} onPress={() => navigateTo("task_detail", {id: item.id, onBack: onRefresh})} style={[{...layout.list.absence_list, borderColor: color_pick }, (now_status) ? { borderWidth: 2 } : null]}>
    ...
    return (
      <View style={layout.container.general}>
        ...
        <Head type="detail" title={'Tugas - '+whoseTask} navigation={props.navigation} noBorder={true} 
          // right_content={(
        ...
        <TouchableOpacity onPress={() => navigateTo('form_tasks', {type: "add", onBack: onRefresh})} style={{ ...layout.button.primary, ...layout.button.wrapper, ...layout.button.small, ...styles.floating_btn }}>
            <Image source={require('@assets/fingerprint_white.png')} style={layout.button.icon} />
            <Text style={{ ...layout.button.text, ...layout.button.text_primary }}>Tambah Tugas</Text>
        </TouchableOpacity>

...
export interface BriefingDetailScreenProps extends NavigationScreenProps<{}> {
}

export const BriefingDetailScreen: React.FunctionComponent<BriefingDetailScreenProps> = observer((props) => {
  ...
  const startWorking = async () => {
    ...
    if (result.kind != "ok") {
      ...
      rootStore.pushData('my_queues', {
        ...
        action: 'startTask',
        will_update: "tasks",
        description: "Mulai bekerja pada briefing",
    ...
    if (result.kind == "ok" && result.data) {
      // Reactotron.log(result.data)
      Toast.show("Anda mulai bekerja pada briefing ini");
      goBack();
  ...
  const stopWorking = async () => {
    ...
    if(result.kind == "ok" && result.data){
      // Reactotron.log(result.data)
      Toast.show("Anda berhenti bekerja pada briefing ini");
      goBack();
      props.navigation.state.params.onBack();
    }
  }
  */
  ...
  const deleteTask = async () => {
    ...
    if (result.kind == "ok" && result.data) {
      // Reactotron.log(result.data)
      Toast.show("briefing berhasil dihapus");
      goBack();
      props.navigation.state.params.onBack();
    }
  }
  ...
  const [routes] = React.useState([
    ...
    { key: 'timelog', title: 'Laporan briefing' },
    { key: 'activity', title: 'History briefing' },
  ]);
  ...
  return (
    <View style={layout.container.general}>
      ...
      >
        ...
        {/*
          <Head type="detail" title={'Detail briefing'} navigation={props.navigation} noBorder={true} opsi={() => togglePopup() } />
          */}
        ...
        {(JSON.parse(rootStore.showCurrentUser("permissions")).manage_task == "0" &&
          <Head type="detail" title={'Detail briefing'} navigation={props.navigation} noBorder={true} />
        )}

        <Animatable.View ref={handleViewRef} style={{ ...layout.box.wrapper, ...layout.box.shadow, ...layout.header_option.container }}>
          <TouchableOpacity onPress={() => navigateTo('form_briefing', { type: "edit", id: task.id, task: task, onBack: onRefresh })} style={{ ...layout.header_option.wrapper }}>
            <Text style={layout.typography.body}>Edit briefing</Text>
          </TouchableOpacity>
            ...
            <View style={{ ...layout.container.wrapper, ...layout.container.bodyView, paddingTop: 20 }}>
              ...
              {(task.is_requires_gps == 1 &&
                <View style={{ ...layout.alert.wrapper, ...layout.alert.info_alt }}>
                  <Text style={{ ...layout.alert.text, ...layout.alert.text_info_alt }}>briefing ini mengharuskan anda untuk mengaktifkan GPS. </Text>
                </View>
              )}

              {(task.is_requires_camera == 1 &&
                <View style={{ ...layout.alert.wrapper, ...layout.alert.info_alt }}>
                  <Text style={{ ...layout.alert.text, ...layout.alert.text_info_alt }}>briefing ini mengharuskan anda untuk mengaktifkan kamera. </Text>
                </View>
              )}

              <Text style={{ ...layout.typography.h4, ...styles.title_header }}>Tentang briefing</Text>
              <View style={{ ...layout.box.wrapper, ...layout.box.shadow, ...layout.box.section }}>
                ...
                <View style={{ ...styles.row, ...layout.list_row.container }}>
                  <Text style={{ ...layout.typography.body, ...layout.list_row.list_label }}>Pemberi briefing</Text>
              ...
              <Text style={{ ...layout.typography.h4, ...styles.title_header }}>Deskripsi briefing</Text>
              <View style={{ ...layout.box.wrapper, ...layout.box.shadow, ...layout.box.section }}>
        ...
        >
          <View style={{ ...layout.bottom_notif.wrapper }}>
            <Text style={layout.bottom_notif.title}>Hapus briefing</Text>
          </View>
          <View style={{ ...layout.bottom_notif.wrapper, ...layout.bottom_notif.row, marginBottom: 50 }}>
            ...
            <Text style={layout.bottom_notif.text}>Apakah anda yakin anda akan menghapus briefing ini?</Text>
          </View>

...
export interface TasksScreenProps extends NavigationScreenProps<{}> {
}

export const TasksScreen: React.FunctionComponent<TasksScreenProps> = observer((props) => {
    ...
    const loadTasks = async (search, additional) => {
      ...
      if(additional){
        ...
        param = {
          ...param,
          is_meeting: (props.navigation.state.params.type == 'task') ? 0 : 1
        };

        param.tugas_yang_saya_berikan = isMine;
      }
          ...
          <ScrollView 
            ...
            <View style={{ ...layout.container.wrapper, ...layout.container.bodyView }}>
              ...
              {datas.length > 0 && datas.map((item,i) => {
                ...
                return(
                  <TouchableOpacity key={slugnow+item.id} onPress={() => navigateTo((props.navigation.state.params.type == 'task') ? 'task_detail' : 'briefing_detail', {id: item.id, onBack: onRefresh})} style={[{...layout.list.absence_list, borderColor: color_pick }, (now_status) ? { borderWidth: 2 } : null]}>
                    <View style={{flexDirection: "row", alignItems: "center", justifyContent: "space-between"}}>
                      ...
                      {(item.new_chat_orang_kepercayaan == true) &&
                        <Image
                          source={require("@assets/ic_alert.png")}
                          style={{
                            width: 30,
                            height: 30,
                            resizeMode: "contain",
        ...
        >
            ...
            <ScrollView style={{ ...layout.bottom_notif.wrapper, marginBottom: 50 }}>
                ...
                */}
                ...
                <Text style={{ ...layout.typography.h3, marginBottom: 10 }}>Tugas Yang Saya Berikan</Text>
                <View style={{ ...layout.textbox.wrapper, ...layout.textbox.outline }}>
                  <Picker
                    selectedValue={isMine}
                    onValueChange={(itemValue, itemIndex) => setIsMine(itemValue)}
                    style={{ ...layout.dropdown.input }}
                  >
    ...
    return (
      <View style={layout.container.general}>
        ...
        <Head type="detail" title={ ((!!props.navigation.state.params.title) ? props.navigation.state.params.title : 'Tugas')+' - '+whoseTask} navigation={props.navigation} noBorder={true} 
          // right_content={(
        ...
        <TouchableOpacity onPress={() => navigateTo((props.navigation.state.params.type == 'task') ? 'form_tasks' : 'form_briefing', {type: "add", onBack: onRefresh, is_meeting: props.navigation.state.params.type})} style={{ ...layout.button.primary, ...layout.button.wrapper, ...layout.button.small, ...styles.floating_btn }}>
            <Image source={require('@assets/fingerprint_white.png')} style={layout.button.icon} />
            <Text style={{ ...layout.button.text, ...layout.button.text_primary }}>Tambah {props.navigation.state.params.title}</Text>
        </TouchableOpacity>
